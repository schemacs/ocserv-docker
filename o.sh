...
function display_usage() {
  cat <<EOF
Usage: install_server.sh [--hostname <hostname>] [--api-port <port>] [--keys-port <port>] [==show|--help]
...
function generate_certificate_fingerprint() {
  ...
  local CERT_PINNED_PUBKEY
  CERT_PINNED_PUBKEY=$(openssl x509 -in "${SB_CERTIFICATE_FILE}" -pubkey -noout | openssl asn1parse -noout -inform pem -out - | openssl dgst -sha256 -binary | openssl base64)
  output_config "pinnedPubkey:$CERT_PINNED_PUBKEY"
}
...
"
  if command -v ufw && ufw status|grep -i 'status: active' >/dev/null; then
      #ufw disable
      ufw allow "${API_PORT}/tcp" || true
      ufw allow "${ACCESS_KEY_PORT}" || true
  fi
  # firewall-cmd --state
  if systemctl is-active --quiet firewalld; then
      # systemctl disable --now firewalld
      firewall-cmd --permanent --add-port="{${API_PORT}/tcp,${ACCESS_KEY_PORT}/tcp,${ACCESS_KEY_PORT}/udp}" || true
      firewall-cmd --reload
  fi
}
...
END_OF_SERVER_OUTPUT
  ...
  command -v yum && yum install epel-release --quiet --assumeyes
  # command -v yum && yum update --quiet --assumeyes
...
function main() {
  ...
  declare -i FLAGS_SHOW=0
  if [[ "$(id -u)" != "0" ]]; then
      echo 'Run as root or with sudo'
      return
  fi
  parse_flags "$@"
  if (( FLAGS_SHOW != 0 )); then
      show
  else
      install_shadowbox
  fi
